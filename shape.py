...
class Shape(object):
	def __init__(self, mat=numpy.array([[True, True, True, True]])):
		if(str(type(mat))=="<class 'numpy.ndarray'>"):
			...
			raise Exception('Expected matrix, found ' + str(mat) + ' a ' + str(type(mat)))
	
	def fliplr(self):
		for row in range(self.matrix.shape[0]):
			self.matrix[row] = [self.matrix[row][i] for i in range(self.matrix.shape[1]-1,-1,-1)]
	...
	def __str__(self):
		...
		for row in range(self.matrix.shape[0]):
			for col in range(self.matrix.shape[1]):
				if(self.matrix[row][col]): string = string + '\u25A1 '
				else: string = string + '  '
			string = string + '\n'
		return string
